...
int CFile::openFile(const char *path)
{
    ...
    if (mFd < 0) {
        ...
        if (strlen(openPath) <= 0) {
            printf("openFile openPath is NULL, path:%s\n", path);
            return -1;
        }
...
int CFile::copyTo(const char *dstPath)
{
    ...
    if (mFd == -1) {
        if ((mFd = open(mPath, O_RDONLY)) == -1) {
            printf("Open %s Error:%s/n", mPath, strerror(errno));
            return -1;
        }
    }

    if ((dstFd = open(dstPath, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR)) == -1) {
        printf("Open %s Error:%s/n", dstPath, strerror(errno));
    }
...
int CFile::delFile(const char *path)
{
    ...
    if (unlink(path) != 0) {
        printf("delete file(%s) err=%s", path, strerror(errno));
        return -1;
    }
    return 0;
}
...
int CFile::delFile()
{
    ...
    if (unlink(mPath) != 0) {
        printf("delete file(%s) err=%s", mPath, strerror(errno));
        return -1;
    }
    return 0;
}
