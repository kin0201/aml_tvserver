...
int CIniFile::LoadFromFile(const char *filename)
{
    ...
    while (fgets (lineStr, MAX_INI_FILE_LINE_LEN, m_pIniFile) != NULL) {
        ...
        //printf("getline=%s len=%d type=%d", lineStr, strlen(lineStr), pLINE->type);
...
int CIniFile::SaveToFile(const char *filename)
{
    ...
    if (filename == NULL) {
        if (strlen(mpFileName) == 0) {
            printf("error save file is null");
            return -1;
        } else {
            filepath = mpFileName;
        }
    ...
    if ((pFile = fopen (filepath, "wb")) == NULL) {
        printf("%s: open %s error(%s)", __FUNCTION__, filepath, strerror(errno));
        return -1;
    }
...
SECTION *CIniFile::getSection(const char *section)
{
    ...
    printf("not find section: %s", section);
    return NULL;
}
